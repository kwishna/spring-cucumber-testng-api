 ...
 */
@Component
public class AppiumClientV1 {
    private static final Logger log = LogManager.getLogger(AppiumClientV1.class);
...
// ═══════════════════════════════════════════════════════════════════════════ // Android Driver Builder // ═══════════════════════════════════════════════════════════════════════════
    ...
    public static class AndroidDriverBuilder {
        private final AppiumClientV1 client;
        private final DesiredCapabilities capabilities = new DesiredCapabilities();
        private String serverUrl;

        private AndroidDriverBuilder(AppiumClientV1 client) {
            this.client = client;
...
// ═══════════════════════════════════════════════════════════════════════════ // iOS Driver Builder // ═══════════════════════════════════════════════════════════════════════════
    ...
    public static class IOSDriverBuilder {
        ...
        private IOSDriverBuilder(AppiumClientV1 client) {
            this.client = client;
            this.serverUrl = client.serverUrl;
...
//            capabilities.setCapability(IOSMobileCapabilityType.WDA_LOCAL_PORT, port);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
// Gesture Handler (Enhanced with Fluent Chaining & Utilities)
// ═══════════════════════════════════════════════════════════════════════════
    ...
    public static class GestureHandler {
        ...
        public GestureHandler waitForGesture(long ms) {
            try {
                Thread.sleep(ms);
                log.info("Waited for {} ms between gestures", ms);
            } catch (InterruptedException ignored) {}
            return this;
        }

        public Point getElementCenter(By locator) {
        ...
        public GestureHandler tapElement(By locator) {
            Point p = getElementCenter(locator);
            return tap(p.getX(), p.getY());
        }

        public GestureHandler tapElement(By locator, int offsetX, int offsetY) {
            WebElement element = driver.findElement(locator);
            int x = element.getLocation().getX() + offsetX;
            int y = element.getLocation().getY() + offsetY;
            return tap(x, y);
        }

        public GestureHandler gestureOnElement(By locator, Runnable gestureAction) {
            WebElement el = driver.findElement(locator);
            log.info("Performing gesture on element: {}", locator);
            gestureAction.run();
            return this;
        }

        public GestureHandler retryGesture(Runnable gesture, int maxAttempts) {
            for (int i = 0; i < maxAttempts; i++) {
                try {
                    gesture.run();
                    log.info("Gesture succeeded on attempt {}", i + 1);
                    return this;
                } catch (Exception e) {
                    log.warn("Gesture failed on attempt {}: {}", i + 1, e.getMessage());
                }
            }
            throw new MobileTestException("Gesture failed after " + maxAttempts + " attempts");
        }

        // ───────────────────────────── Swipe Gestures ─────────────────────────────

        public GestureHandler swipeUp() { return swipeUp(0.8, 0.2); }
        public GestureHandler swipeDown() { return swipeDown(0.2, 0.8); }
        public GestureHandler swipeLeft() { return swipeLeft(0.8, 0.2); }
        public GestureHandler swipeRight() { return swipeRight(0.2, 0.8); }
        ...
        public GestureHandler scrollToTop() { return swipeDown(0.3, 0.9); }

        public GestureHandler scrollToBottom() { return swipeUp(0.9, 0.2); }
        ...
        public GestureHandler doubleTap(int x, int y) {
            tap(x, y).waitForGesture(100).tap(x, y);
            log.info("Double tapped at ({},{})", x, y);
            return this;
        }
        ...
        public GestureHandler dragAndDrop(int startX, int startY, int endX, int endY) {
            PointerInput finger = new PointerInput(PointerInput.Kind.TOUCH, "finger");
            ...
            log.info("Dragged from ({}, {}) to ({}, {})", startX, startY, endX, endY);
            return this;
        }

        public GestureHandler twoFingerSwipeDown() {
            int startX1 = screenSize.width / 3;
            int startX2 = 2 * screenSize.width / 3;
            int startY = (int) (screenSize.height * 0.2);
            int endY = (int) (screenSize.height * 0.8);
            ...
            Sequence s1 = new Sequence(f1, 1);
            s1.addAction(f1.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), startX1, startY));
            s1.addAction(f1.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
            s1.addAction(f1.createPointerMove(Duration.ofMillis(800), PointerInput.Origin.viewport(), startX1, endY));
            s1.addAction(f1.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));

            Sequence s2 = new Sequence(f2, 1);
            s2.addAction(f2.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), startX2, startY));
            s2.addAction(f2.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
            s2.addAction(f2.createPointerMove(Duration.ofMillis(800), PointerInput.Origin.viewport(), startX2, endY));
            s2.addAction(f2.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));

            driver.perform(Arrays.asList(s1, s2));
            log.info("Performed two-finger swipe down");
            return this;
        }

        // ───────────────────────────── Pinch / Zoom ─────────────────────────────

        public GestureHandler pinchIn() { return pinch(false, 150); }
        public GestureHandler pinchOut() { return pinch(true, 150); }

        private GestureHandler pinch(boolean zoomIn, int offset) {
            ...
            int startX1 = zoomIn ? cx : cx - offset;
            int startX2 = zoomIn ? cx : cx + offset;
            int endX1 = zoomIn ? cx - offset : cx;
            int endX2 = zoomIn ? cx + offset : cx;

            Sequence s1 = new Sequence(f1, 1);
            s1.addAction(f1.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), startX1, cy));
            s1.addAction(f1.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
            s1.addAction(f1.createPointerMove(Duration.ofMillis(500), PointerInput.Origin.viewport(), endX1, cy));
            s1.addAction(f1.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));

            Sequence s2 = new Sequence(f2, 1);
            s2.addAction(f2.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), startX2, cy));
            s2.addAction(f2.createPointerDown(PointerInput.MouseButton.LEFT.asArg()));
            s2.addAction(f2.createPointerMove(Duration.ofMillis(500), PointerInput.Origin.viewport(), endX2, cy));
            s2.addAction(f2.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));

            driver.perform(Arrays.asList(s1, s2));
            log.info("Performed {}", zoomIn ? "pinch out (zoom in)" : "pinch in (zoom out)");
            return this;
        }

        // ───────────────────────────── Fling & Edge ─────────────────────────────

        public GestureHandler flingUp() { return fling(0.8, 0.2); }
        public GestureHandler flingDown() { return fling(0.2, 0.9); }

        private GestureHandler fling(double startYPercent, double endYPercent) {
            int x = screenSize.width / 2;
            int startY = (int) (screenSize.height * startYPercent);
            int endY = (int) (screenSize.height * endYPercent);
            swipe(x, startY, x, endY, 300);
            log.info("Performed fling");
            return this;
        }

        public GestureHandler edgeSwipeFromLeft() {
            swipe((int) (screenSize.width * 0.05), screenSize.height / 2,
                    (int) (screenSize.width * 0.5), screenSize.height / 2, 500);
            log.info("Edge swipe from left");
            return this;
        }

        public GestureHandler edgeSwipeFromRight() {
            swipe((int) (screenSize.width * 0.95), screenSize.height / 2,
                    (int) (screenSize.width * 0.5), screenSize.height / 2, 500);
            log.info("Edge swipe from right");
            return this;
        }
    }
